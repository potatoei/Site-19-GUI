...
-- Below is the Adonis Anticheat, if you have an executor that does not support atleast 90% UNC,
-- Or if functions do not work after executing, please comment the below loadstring by using Ctrl + /
...
Credits.Text = "Toon, SCP-079, Moon"
...
local readfile = type(readfile) == "function" and function(file, data, safe)
	if safe == true then return pcall(readfile, file) end
	return readfile(file)
end
...
function toClipboard(txt)
	if everyClipboard then
		...
		warn("Your exploit does not support the clipboard function.")
	end
end
		...
		if pcall(function() readfile("IY_FE.iy") end) then
			if readfile("IY_FE.iy") ~= nil then
				...
				if not success then
					...
					writefileCooldown("IY_FE.iy", defaults, true)
					wait()
					saves()
				end
			else
...
function sendChatWebhook(player, message)
	if httprequest and vtype(logsWebhook, "string") then
		local log = HttpService:JSONEncode(
			{
				content = message,
				avatar_url = "https://files.catbox.moe/i1968v2.jpg",
				username = formatUsername(player),
				allowed_mentions = {parse = {}} -- disable mentions
			}
		)
...
local CleanFileName = function(str)
	return tostring(str:gsub("[^%w%s]", ""):gsub("%s+", " "):sub(1, 175))
end
...
CMDs[#CMDs + 1] = {NAME = 'oldconsole', DESC = 'Loads the old oblox console'}
...
CMDs[#CMDs + 1] = {NAME = 'antichatlogs / anticheatlogger', DESC = 'Prevents Roblox from seeing your chat messages {Legacy Chat}'}
CMDs[#CMDs + 1] = {NAME = 'joinlogs / jlogs', DESC = 'Log when people join'}
...
CMDs[#CMDs + 1] = {NAME = 'loopxray', DESC = 'Loops xraying'}
CMDs[#CMDs + 1] = {NAME = 'unloopxray', DESC = 'Unloops exraying'}
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'v / view [plr]', DESC = 'View a player (v is short for view)'}
...
CMDs[#CMDs + 1] = {NAME = 'fakeout', DESC = 'Teleport to the void then come back'}
CMDs[#CMDs + 1] = {NAME = 'antivoid', DESC = 'Prevents you from falling into the void'}
CMDs[#CMDs + 1] = {NAME = 'unantivoid', DESC = 'Disables antivoid'}
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = 'gravity / grav [num] (CLIENT)', DESC = 'Changes your gravity (default is approx. 196)'}
CMDs[#CMDs + 1] = {NAME = 'sit', DESC = 'Makes your character sit'}
CMDs[#CMDs + 1] = {NAME = 'unsit', DESC = 'Makes your character not sit'}
...
CMDs[#CMDs + 1] = {NAME = 'nohead / headless', DESC = 'Removes your head (NOT WORKING, ARCHIVED)'}
...
CMDs[#CMDs + 1] = {NAME = 'vr', DESC = 'Loads CLOVR by Abacaxl, Archived, will most likely not work'}
...
CMDs[#CMDs + 1] = {NAME = 'mouseteleport', DESC = 'Teleports your character to your mouse, best used as a keybind'}
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = 'boxreach', DESC = 'Increases the hitbox of your held tool in a box shape'}
...
CMDs[#CMDs + 1] = {NAME = 'addallplugins / loadallplugins', DESC = 'Adds all the plugins'}
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = '049HP', DESC = 'Shows 049 HP'}
CMDs[#CMDs + 1] = {NAME = '1048HP', DESC = 'Shows 1048 HP'}
...
SpecialPlayerCases = {
	...
	["cursor"] = function(speaker) -- cursor
		local plrs = {}
		local v = GetClosestPlayerFromCursor()
		if v ~= nil then table.insert(plrs, v) end
		return plrs
...
-- Above is the mobile fly function :)
...
addcmd('toggleinvis',{},function(args, speaker)
	execCmd(invisRunning and "vislbe" or "invisible")
end)
...
addcmd('maxslopeangle',{'msa'},function(args, speaker)
	...
	if isNumber(sangle) then
		speaker.Character:FindFirstChildOfWhichIsA('Humanoid').MaxSlopeAngle = sangle
	end
end)
...
addcmd('dance', {}, function(args, speaker)
	if not r15(speaker) then
		...
		animTrack = speaker.Character:FindFirstChildWhichIsA('Humanoid'):LoadAnimation(animation)
		animTrack:Play()
...
addcmd('unsit',{},function(args, speaker)
	if speaker.Character:FindFirstChildWhichIsA("Humanoid").Sit == true then
...
addcmd('sitwalk',{},function(args, speaker)
	...
	if r15(speaker) then
		...
		speaker.Character:FindFirstChildWhichIsA('Humanoid').HipHeight = -1.5
	end
end)
...
addcmd('edgejump',{'ejump'},function(args, speaker)
	local Char = speaker.Character
	local Human = Char and Char:FindFirstChildWhichIsA("Humanoid")
	-- Full credit to NoelGamer06 @V3rmillion, now v3rm.net
	local state
	local laststate
	local lastcf
...
addcmd('clickteleport',{},function(args, speaker)
	if speaker == Players.LocalPlayer then
		notify('please go to settings>keybinds>iforgot')
	end
end)
...
addcmd('clickdelete',{},function(args, speaker)
	notify('Go to settings>Keybinds>Add Click Delete')
end)
...
addcmd('instantproximityprompts',{'instantpp'},function(args, speaker)
	-- Function to set HoldDuration of a ProximityPrompt to 0
    local function setHoldDurationToZero(prompt)
...
addcmd('vr',{},function(args, speaker)
	-- Full credit to Abacaxl @V3rmillion, archived
	notify("Loading", "Hold on a sec")
...
addcmd("flyfling", {}, function(args, speaker)
    ...
    wait()
    execCmd("vehiclefly\\flyfling")
end)

addcmd("unflyfling", {}, function(args, speaker)
    execCmd("unvehiclefly\\unfling\\unwalkfling\\breakvelocity")
end)
...
-- Below are the custom-made commands...

addcmd('173HP',{''},function(args, speaker)
    local SCPOneSeven = game.Workspace.SCP["173"].Body
    local randomherescpscpscp = SCPOneSeven:GetAttribute("Health")
	notify(randomherescpscpscp .. "HP")
end)

addcmd('049HP',{''},function(args, speaker)
    notify(game.Workspace.SCP["049"].Humanoid.Health .. "HP")
end)
addcmd('096HP',{''},function(args, speaker)
    notify(game.Workspace.SCP["096"].Humanoid.Health .. "HP")
end)
addcmd('682HP',{''},function(args, speaker)
    notify(game.Workspace.SCP["682"].Humanoid.Health .. "HP")
end)
        ...
        if v ~= speaker then
            ...
            if player then
                for _, team in ipairs(Teams:GetTeams()) do
                    if team:IsA("Team") and team.Name:lower():find(targetTeamLower, 1, true) then
                        player.Team = team 
                        break
                    end
                end
            else
                notify("Player '" .. v .. "' not found.")
            end
        else
            ...
            if player then
                ...
                notify("Player '" .. speaker .. "' not found.")
            end
        end
    end
end)
...
getgenv().gearkey = Enum.KeyCode.N
addcmd('GearVision', {'NightVision'}, function(args, speaker)
    ...
    userInputService.InputBegan:Connect(function(input, gameProcessedEvent)
        if not gameProcessedEvent and input.KeyCode == getgenv().gearkey then
...
addcmd('CDtools', {''}, function(args, speaker)
    ...
    game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("PrivateEvent"):FireServer(unpack(iargs))
    ...
    game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("PrivateEvent"):FireServer(unpack(iiiargs))
end)


addcmd('AD', {'ADept'}, function(args, speaker)
    wait(0.2)
    local iiargs = {
    [1] = "SetTeam",
    [2] = game:GetService("Players").LocalPlayer,
    ...
    game:GetService("ReplicatedStorage"):WaitForChild("Events"):WaitForChild("PrivateEvent"):FireServer(unpack(iiargs))
end)


addcmd('DOADirector', {'DOA', 'DOAD'}, function(args, speaker)
    game.Players.LocalPlayer.Character.Pants.PantsTemplate = "rbxassetid://8134408475"
    game.Players.LocalPlayer.Character.Shirt.ShirtTemplate = "rbxassetid://8134403498"
end)


addcmd('BM9s', {'Beretta M9s'}, function(args, speaker)
    if game.Players.LocalPlayer.Team.Name == "Administrative Department" then
        local nargs = {
            [1] = "GiveTool",
            [2] = game:GetService("Players").LocalPlayer,
            [3] = "Beretta M9s"
...
addcmd('Coffee', {'CMachine'}, function(args, speaker)
    local playery = game:GetService("Players").LocalPlayer
    local playerGuyi = playery.PlayerGui
    local hudGuyi = playerGuyi.HudGui
    local screen = hudGuyi["#screen"]

    local function setDescendantsVisible(parent)
        for _, descendant in ipairs(parent:GetDescendants()) do
            if (descendant:IsA("Frame") or descendant:IsA("TextLabel") or descendant:IsA("ImageLabel")) and descendant.Name ~= "UIScale" then
            descendant.Visible = true
            end
        end
    end
    screen.Visible = true
    setDescendantsVisible(screen)
end)


addcmd('DoctorGears', {''}, function(args, speaker)
    game.Players.LocalPlayer.Character.Shirt.ShirtTemplate = "rbxassetid://11917692335"
    game.Players.LocalPlayer.Character.Pants.PantsTemplate = "rbxassetid://11917693322"
end)

addcmd('RRHUniform', {''}, function(args, speaker)
    speaker.Character.Shirt.ShirtTemplate = "rbxassetid://11888149047"
...
addcmd('makehp', {''}, function(args, speaker)
	speaker.Character.Humanoid.Health = args[1]
end)

addcmd('notifyhp', {''}, function(args, speaker)
	notify(speaker.Character.Humanoid.Health)
end)
...
addcmd('config', {'configuration'}, function(args, speaker)
        ...
        for i,v in pairs(getgc(true)) do
            if type(v) == 'table' and rawget(v, 'Spread') then
                if v.Rpm == 3000 then
                    ...
                    v.Spread = 0
                    v.Automatic = true
                elseif v.Rpm == 3000 and speaker.Backpack:FindFirstChild("Kriss Vector") then
                    v.Rpm = 1100
                    v.Spread = 0
                    v.Automatic = true
                else
    ...
    elseif args[1] == "2" and game.PlaceId == 1440936008 then
        for i,v in pairs(getgc(true)) do
            if type(v) == 'table' and rawget(v, 'Spread') then
                v.Spread = 0
                v.Automatic = true
    ...
    elseif game.PlaceId == 12299094705 or game.PlaceId == 13598279407 and args[1] == "1" then
        for i, v in pairs(getgc(true)) do
            if type(v) == "table" and rawget(v, 'BaseDamage') then
                ...
                v.Piercing = 1
                v.Lifesteal = 90
                v.HeadshotDamageMultipler = 5
                v.FlamingBullet = true
                v.ReloadTime = 1
            end
        end
    elseif game.PlaceId == 6068696524 and args[1] == '1' then
        for i, v in pairs(getgc(true)) do
            if type(v) == "table" and rawget(v, "headshot") then
                v.headshot = math.huge
                v.bodyshot = math.huge
            end
        end
    elseif args[1] == "" then
        notify("Configuration not found, arguments must be provided.")
    elseif game.PlaceId ~= 6068696524 or 12299094705 or 13598279407 or 1440936008 then
...
task.spawn(function()
	...
	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify('Outdated','Get a new version or change the version text.')
		end
